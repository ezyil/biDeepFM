...
def biDCN(feature_dim_dict, embedding_size=8,
    ...
    """Instantiates the Multi-objective Deep&Cross Network architecture.
    ...
    if len(dnn_hidden_units) > 0 and cross_num > 0:  # Deep & Cross
        ...
        final_logit_1 = tf.keras.layers.Dense(1, use_bias=False, activation=None)(stack_out)
    elif len(dnn_hidden_units) > 0:  # Only Deep
        deep_out = DNN(dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout,
                       dnn_use_bn, seed)(deep_input)
        final_logit = tf.keras.layers.Dense(1, use_bias=False, activation=None)(deep_out)
        final_logit_1 = tf.keras.layers.Dense(1, use_bias=False, activation=None)(deep_out)
    elif cross_num > 0:  # Only Cross
        cross_out = CrossNet(cross_num, l2_reg=l2_reg_cross)(deep_input)
        final_logit = tf.keras.layers.Dense(1, use_bias=False, activation=None)(cross_out)
        final_logit_1 = tf.keras.layers.Dense(1, use_bias=False, activation=None)(cross_out)
    else:  # Error
